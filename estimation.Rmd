...
c19 = read.csv(paste0(dir, "catch2019.csv"), fileEncoding = fileEncoding) %>% select(-X)
...
summary(catch_new)
# write.csv(catch_new, "catch2020.csv", fileEncoding = fileEncoding)
...
# step 3; CPUE trend ----------------------------------------------------------
# -2018
gyo_old = read.csv(paste0(dir, "okisoko_old.csv"), fileEncoding = fileEncoding)
...
total_catch_pref_old = read.csv(paste0(dir, "pref_catch_old.csv"), fileEncoding = fileEncoding)
...
table_s2 = data.frame(year = rep((n_year-5):n_year+1), 資源量_ton = abind(trend %>% dplyr::filter(year > (n_year-5)) %>% select(total), total_biomass_next, along = 1), 親魚量 = abind(srr %>% filter(year2 > (n_year-5)) %>% mutate(biomass2 = biomass/1000000) %>% select(biomass2), NA, along = 1), 漁獲量_ton = abind("not_change", "not_change", "not_change", total_catch_pref, catch_this, NA, along = 1), F = abind(fishing_trend %>% filter(year > (n_year-5), data == "f") %>% select(value), f_current, NA, along = 1), 漁獲割合 = abind(fishing_trend %>% filter(year > (n_year-5), data == "rate") %>% select(value), rate_this, NA, along = 1))
...
fig_a31a = g + geom_polygon(data = japan2, group = japan$group, fill= "gray50", colour= "gray50")  + coord_map(xlim = c(140.5, 143), ylim = c(36, 42)) + stat_contour(aes(z=depth),binwidth=200,colour="black")+theme_bw(base_family = "HiraKakuPro-W3")+th+geom_point(data = trawl_length3, aes(x = lon, y = lat, shape = station_code), size = 3)+scale_shape_manual(values = c(15, 4, 17, 8, 18, 16, 1, 2))+labs(title = "(A)", x = "経度", y = "緯度", shape = "調査ライン")
...
setwd(dir_output)
# write.csv(catch_t1, "catch_t1.csv", fileEncoding = fileEncoding)
# write.csv(catch_t2, "catch_t2.csv", fileEncoding = fileEncoding, row.names=F)
write.csv(catch_new, "catch2020.csv", fileEncoding = fileEncoding, row.names=F) #来年の資源評価で使うデータ
write.csv(merge, "table1.csv", fileEncoding = fileEncoding, row.names=F)
write.csv(catch_t3, "table2.csv", fileEncoding = fileEncoding, row.names=F)
...
write.csv(length, "tableA4-2.csv", fileEncoding = fileEncoding, row.names=F)
write.csv(weight, "tableA4-3.csv", fileEncoding = fileEncoding, row.names=F)
...
write.csv(fishing_trend %>% dplyr::rename(label = data2) %>% select(-data), "table9.csv", fileEncoding = fileEncoding, row.names=F) 
...
# ggsave(file = "figA3-1B.png", plot = fig_a31b, units = "in", width = 11.69, height = 8.27)
# ggsave(file = "figA3-1C.png", plot = fig_a31c, units = "in", width = 11.69, height = 8.27)
layout1 <- rbind(c(1, 2),
                 c(1, 3))
# fig_a31 = grid.arrange(fig_a31a, fig_a31b, fig_a31c, layout_matrix = layout1)
...
ggsave(file = "figA3-2.png", plot = fig_a32, units = "in", width = 11.69, height = 8.27)
...
# ggsave(file = "figA4-1.png", plot = fig_a41, units = "in", width = 11.69, height = 8.27)
png("ALK.png")
...
okisoko_all %>% filter(year == (n_year-1))

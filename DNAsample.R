...
# all survey stations ------------------------------------------------
setwd("/Users/Yuki/Dropbox/業務/キチジゲノム")
df = read.csv("loc_DNAsample.csv", fileEncoding = "CP932") %>% mutate(lon = lon1+lon2/60, lat = lat1+lat2/60) %>% select(lon, lat)
summary(df)
...
labs = labs(x = "経度", y = "緯度", colour = "漁獲量（トン）")
fig = local_map+theme_bw(base_family = "HiraKakuPro-W3")+th+p+labs
map = local_map+theme_bw(base_family = "HiraKakuPro-W3")+th+labs

ggsave(file = "DNAsample_all.png", plot = fig, unit = "cm", width = 6.88, height = 11.49)
ggsave(file = "map.png", plot = map, unit = "cm", width = 6.88, height = 11.49)

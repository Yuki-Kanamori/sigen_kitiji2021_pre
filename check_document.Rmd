...
# step 1; catch trend -----------------------------------------------------
catch_old = read.csv(paste0(dir, "catchdata_old.csv"), fileEncoding = "CP932") %>% na.omit()
...
c19 = read.csv(paste0(dir, "catch2019.csv"), fileEncoding = "CP932") %>% select(-X)
...
```{r okisoko, echo=F, warning=F, message=F, results=F}
# -------------------------------------------------------------------------
# 2-3  漁獲量まとめ
#      (引き継ぎ資料の2-3部分)
# -------------------------------------------------------------------------
...
# step 2; effort trend ----------------------------------------------------
# ~2018まで
eff_old = read.csv(paste0(dir, "effortdata_old.csv"), fileEncoding = "CP932")
eff_old = ddply(eff_old, .(method, year), summarize, sum = sum(effort))

#2019
oki2019 = read.xlsx(paste0(dir, "okisoko_after2019.xlsx"), sheet = "2019")
...
# step 3; CPUE trend ----------------------------------------------------------
# -2018
gyo_old = read.csv(paste0(dir, "okisoko_old.csv"), fileEncoding = "CP932")
...
```{r get biomass and N, echo=FALSE, warning=FALSE, message=FALSE, results='hide'}
# -2018
...
# weight is from the survey of last Oct. (i.e., the year is different between est and weight)
# est = estimated abundance in Jan.
...
```{r fig4, echo=F, warning=F, message=F, results=F}
# 3-1 図4: 漁場の空間分布 --------------------------------------------------------------
x <- read.delim(paste0(dir, "Map.txt"), header=T)

###地図の入ったテキストファイル###
z <- read.table(paste0(dir, "キアンコウ緯度経度.txt"), header=T, fileEncoding = "CP932")
z2 = read.xlsx(paste0(dir, "okisoko_after2019.xlsx"), sheet = "2019")
...
```{r figsA3, echo=F, warning=F, message=F, results=F}
# ---------------------------------------------------------------
# 3-5  補足図3-1; 調査地点，密度分布，及び体長組成 ---------------------------------
# ---------------------------------------------------------------
trawl_length = read.xlsx(paste0(dir, "q_魚種別体長別資源量2020.xlsx")) %>% filter(和名 == "キチジ")
...
for(i in 1:length(sheets)){
  ...
  len_num = read.csv(paste0(dir, "survey_N_at_length.csv"), fileEncoding = "CP932") %>% mutate(year = as.numeric(str_sub(調査種類名称, 1, 4))) %>% filter(year == as.numeric(sheets[i])) %>% select(-year)
  len_num = len_num[, 16:ncol(len_num)] %>% mutate(site = c("N", "S"))
...
```{r fig9, echo=F, warning=F, message=F, results=F}
# # 3-2  図9; 漁獲物の体長組成  ------------------------------------------
...
# (1-B) きちじとこきちじの漁獲量---------------------------------------------------------
g_miya = read.csv(paste0(dir, "catch_miyagi.csv"), fileEncoding = "CP932")
#summary(g_miya)
...
# (1-C) こきちじの体長組成---------------------------------------------------------
tai_miya = read.csv(paste0(dir, "taityo_miyagi.csv"), fileEncoding = "CP932")
#summary(tai_miya)
...
# (1-D) きちじの体長組成---------------------------------------------------------
yatyo = read.csv(paste0(dir, "yatyo_miyagi.csv"), fileEncoding = "CP932")
...
# (3) 八戸 ------------------------------------------------------------------
tai_hati = read.csv(paste0(dir, "hati_sokutei.csv"), fileEncoding = "CP932")

# ファイルの規格コードが変なので，修正
code_data = read.csv(paste0(dir, "hati_sokutei2019.csv"), fileEncoding = "CP932")
...
# pとs
ps = read.csv(paste0(dir, "seimitu.csv"), fileEncoding = "CP932") %>% mutate(season = ifelse(between(month, 1, 6), "1-6", "7-12"), tag = paste(taityo, meigara, month, total_number_in_box, sep = "_"), tag_box = paste(month, total_number_in_box,meigara,  sep = "_"), taityo2 = taityo%/%10, meigara = ifelse(meigara == "SS", "P", "S"))
...
```{r save, echo=F, warning=F, message=F, results=F}
dir.create(paste0(dir, "output"))
dir_output = paste0(dir, "output")
setwd(dir_output)
# write.csv(catch_t1, "catch_t1.csv", fileEncoding = "CP932")
# write.csv(catch_t2, "catch_t2.csv", fileEncoding = "CP932", row.names=F)
write.csv(catch_new, "catch2020.csv", fileEncoding = "CP932", row.names=F) #来年の資源評価で使うデータ
write.csv(merge, "table1.csv", fileEncoding = "CP932", row.names=F)
write.csv(catch_t3, "table2.csv", fileEncoding = "CP932", row.names=F)
write.csv(effort_t1, "table3.csv", fileEncoding = "CP932", row.names=F)
write.csv(table4, "table4.csv", fileEncoding = "CP932", row.names=F)

write.csv(trawl, "tableA4-1.csv", fileEncoding = "CP932", row.names=F)
write.csv(length, "tableA4-2.csv", fileEncoding = "CP932", row.names=F)
write.csv(weight, "tableA4-3.csv", fileEncoding = "CP932", row.names=F)
write.csv(q, "tableA4-4.csv", fileEncoding = "CP932", row.names=F)
write.csv(abund_oct_sel %>% mutate(number_sel_thou = number_sel/1000) %>% dplyr::rename(biomass_sel_ton = biomass_sel) %>% select(-number_sel), "tableA4-5&6.csv", fileEncoding = "CP932", row.names=F)
write.csv(est %>% mutate(number = number/1000) %>% dplyr::rename(number_thou = number, biomass_ton = biomass), "tableA4-7&8.csv", fileEncoding = "CP932", row.names=F) 
write.csv(biomass_female %>% mutate(biomass_ton = biomass/1000000) %>% dplyr::rename(year = year2) %>% select(-biomass), "tableA4-8female.csv", fileEncoding = "CP932", row.names=F)
write.csv(fishing_trend, "tableA4-9.csv", fileEncoding = "CP932", row.names=F)
write.csv(fishing_trend %>% dplyr::rename(label = data2) %>% select(-data), "table9.csv", fileEncoding = "CP932", row.names=F) 

# write.csv(number_at_age %>% dplyr::rename(age_length = age), "number_at_age.csv", fileEncoding = "CP932", row.names=F)
# write.csv(est, "estimated_abundance.csv", fileEncoding = "CP932", row.names=F) 
...
ggsave(file = "figA3-1B.png", plot = fig_a31b, units = "in", width = 8.27, height = 11.69)
ggsave(file = "figA3-1B_2.png", plot = fig_a31b, units = "in", width = 11.69, height = 8.27)
ggsave(file = "figA3-1C.png", plot = fig_a31c, units = "in", width = 11.69, height = 8.27)
ggsave(file = "fig_A3-2.png", plot = fig_a32, units = "in", width = 11.69, height = 8.27)
ggsave(file = "fig_A3-3.png", plot = fig_a33, units = "in", width = 11.69, height = 8.27)
ggsave(file = "fig_A3-4.png", plot = fig_a41, units = "in", width = 11.69, height = 8.27, scale = 0.9)
png("ALK.png")
...
### (5) 再生産関係
```{r rps, echo=F}
srr %>% select(year2, rps) %>% dplyr::rename(year = year2)
```
...
#### 2021年の資源量と資源尾数（3段落目）
```{r stock 2021, message=F}
#資源量
(trend %>% dplyr::rename(ton = total) %>% filter(year == (as.numeric(str_sub(Sys.Date(), 1, 4)))))

#尾数
(est2 %>% dplyr::group_by(year) %>% dplyr::summarize(N_thou = sum(total)/1000) %>% filter(year == (as.numeric(str_sub(Sys.Date(), 1, 4)))))
```
...
### (4) ABCの再評価（表）
<!----
`r (year - 1)`年の資源量は`r (total_biomass_this = sum(abund_abc$biomass_est))`，F limitは`r (total_biomass_this = sum(abund_abc$biomass_est))`，F targetは`r (re_abc_target = (f_target*(1-exp(-z_abc)))/z_abc*total_biomass_this)`．  
漁獲量とFは要約にある．  
--->
#### 2020（3行目）
```{r reABC 2 years ago}
